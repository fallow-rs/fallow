...
/// other `MemberKind`s, so a mis-resolution onto one of those credits nothing.
        ...
        for access in &resolved.member_accesses {
            ...
            for seed_key in seed_keys {
                for factory_origin in
                    walk_re_export_origins(graph, seed_key.file_id, seed_key.export_name.as_str())
                {
                    let Some(factory_module) = module_by_id.get(&factory_origin.file_id) else {
                        continue;
                    };
                    ...
                    let factory_local_keys = factory_keys_cache
                        .entry(factory_origin.file_id)
                        .or_insert_with(|| build_local_to_export_keys(factory_module));
                    let Some(class_seed_keys) =
                        factory_local_keys.get(factory_return.class_local_name.as_str())
                    else {
                        continue;
                    };
                    for class_seed in class_seed_keys {
                        for class_origin in export_key_with_origins(graph, class_seed) {
                            let Some(class_module) = module_by_id.get(&class_origin.file_id) else {
                                continue;
                            };
                            // (4) the resolved export must be a class with members.
                            if !export_is_class_with_members(
                                class_module,
