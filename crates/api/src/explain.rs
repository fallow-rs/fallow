...
fn health_runtime_rule_guide(id: &str) -> Option<RuleGuide> {
    Some(match id {
        ...
        | "fallow/high-complexity" => RuleGuide {
            example: "A function contains several nested conditionals, loops, and early exits, exceeding the configured complexity threshold. fallow also flags synthetic `<template>` findings on markup: Angular .html templates and inline `@Component({ template: ... })` literals, Vue SFC `<template>` blocks, Svelte markup, and Astro markup. `<component>` rollup findings combine the worst class method with its template.",
        ...
        "fallow/high-crap-score" => RuleGuide {
            example: "A complex function has little or no matching Istanbul coverage, so its CRAP score crosses the configured gate. It also fires on synthetic `<template>` findings (Angular, Vue, Svelte and Astro markup), where the coverage term is fixed at zero because a template carries no direct coverage of its own.",
            how_to_fix: "For functions, add focused tests for the risky branches first, then simplify the function if the score remains high. For a `<template>` finding, tests cannot move the score: reduce the number of template branches, or set `maxCrap` for that file through a `health.thresholdOverrides` entry.",
        },
