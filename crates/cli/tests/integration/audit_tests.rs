...
fn run_prune_with_scan_root(args: &[&str], scan_root: &Path) -> crate::common::CommandOutput {
...
fn audit_cache_prune_defaults_root_to_cwd_and_honors_threshold_precedence() {
    ...
    let no_root = Command::new(crate::common::fallow_bin())
        .current_dir(root)
        .env("RUST_LOG", "")
        .env("NO_COLOR", "1")
...
fn audit_with_env(root: &Path, env: &[(&str, &str)]) -> crate::common::CommandOutput {
    let bin = fallow_bin();
...
fn assert_branchy_below_threshold(output: &crate::common::CommandOutput) {
    assert_eq!(
        output.code, 0,
...
fn assert_branchy_inherited(output: &crate::common::CommandOutput) {
    let json = parse_json(output);
...
fn run_walkthrough_human(root: &Path) -> crate::common::CommandOutput {
    run_fallow_raw(&[
        "review",
        "--root",
        root.to_str().unwrap(),
...
fn run_rename_audit(dir: &Path) -> crate::common::CommandOutput {
    run_fallow_raw(&[
        "audit",
        "--root",
        dir.to_str().unwrap(),
...
fn audit_new_only_degrades_to_syntactic_attribution_on_type_aware_identity_mismatch() {
    ...
    let comment = crate::common::run_fallow_raw_with_type_aware_sidecar(&[
        "audit",
        "--root",
        dir.to_str().unwrap(),
        "--base",
...
fn audit_type_aware_base_snapshot_cache_preserves_identity_comparison() {
    ...
    let cold = crate::common::run_fallow_raw_with_type_aware_sidecar(&args);
    let warm = crate::common::run_fallow_raw_with_type_aware_sidecar(&args);
