...
use crate::common::{parse_json, run_fallow_raw, run_fallow_raw_with_env};
...
fn dead_code_json(root: &str, extra: &[&str]) -> crate::common::CommandOutput {
...
fn dead_code_json_with_warnings(root: &str, extra: &[&str]) -> crate::common::CommandOutput {
    let mut args = vec!["dead-code", "--root", root, "--format", "json"];
    args.extend_from_slice(extra);
