...
- [ ] **Four parallel attribution surfaces for path-anchored issue types**: when the review covers a NEW issue type with a path-anchored finding, enumerate these four surfaces and confirm the new type appears in each: (1) `crates/engine/src/changed_files.rs::filter_results_by_changed_files` (`--changed-since` retain); (2) `crates/engine/src/dead_code.rs::apply_rule_severities` per-file override branch AND base branch; (3) `crates/engine/src/effective_severity.rs`: a `RuleSeverity` impl for the new finding wrapper, which the exit code, the CI format severities and the audit ledger all read (the exhaustive visitors in that file do not compile until the new field is listed); (4) `crates/api/src/audit_keys.rs::dead_code_keys` + `retain_introduced_dead_code` + `annotate_dead_code_json` (audit attribution trio). Concrete grep target for each new finding type's array name:
  ```bash
  for file in crates/engine/src/changed_files.rs crates/engine/src/dead_code.rs crates/engine/src/effective_severity.rs crates/api/src/audit_keys.rs; do
    echo "=== $file ==="
    grep -nE "<new_field_name>" "$file" || echo "  MISSING -- BLOCK"
  done
  ```
