...
- `analyze/unused_files.rs`, `unused_exports.rs`, `unused_deps.rs`, `members/` — Per-issue-type detection
...
- No process spawning in analysis crates: fallow-core/extract/graph pin `#![cfg_attr(not(test), deny(clippy::disallowed_methods))]` (banning `Command::new` via `.clippy.toml`, set to `allow` workspace-wide for cli/mcp) and contain zero non-test spawn sites. Analysis must never execute the analyzed project's code. Git invocation lives in `fallow-engine` (`churn.rs`, env-scrubbed via `git_env::clear_ambient_git_env`, test-interceptable via `set_spawn_hook`); `crates/core/src/git_env.rs` keeps the shared `clear_ambient_git_env` helper. Adding a raw `Command::new` to core/extract/graph is a build failure by design. See `SECURITY.md` and the `safe_analysis` integration test.
