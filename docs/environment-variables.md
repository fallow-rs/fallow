...
| `FALLOW_SUGGESTIONS` | Set to `off`/`0`/`false`/`no`/`disabled` to suppress the `next_steps[]` array in JSON output and the human `Next:` line. Useful for CI consumers that snapshot-diff raw `--format json`. With suggestions off, fallow also starts no `git` probe for the next steps. | `on` | `FALLOW_SUGGESTIONS=off` |
...
| `FALLOW_CLOCK_EPOCH` | Reference instant, in unix seconds, that the run measures commit recency and ownership staleness against. Fallow otherwise reads HEAD's committer timestamp, so two runs over one commit already agree; set this to compare two checkouts against one fixed instant, or to pin churn-derived numbers in a project whose history is imported with `--churn-file` rather than read from git. An empty or non-numeric value is ignored with a warning. | HEAD committer timestamp | `FALLOW_CLOCK_EPOCH=1788782400` |
