...
| `FALLOW_TYPE_AWARE_BIN` | Experimental path override for the TypeScript semantic refinement sidecar used by `dead-code --type-aware`. Relative paths resolve from the caller's working directory before `--root` is applied. | discovered | `FALLOW_TYPE_AWARE_BIN=./tools/type-aware-sidecar/fallow-type-aware.mjs` |
