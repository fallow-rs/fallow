...
| `FALLOW_MCP_WARM_SESSION` | MCP server: set to `0`, `false`, `off` or `no` to stop typed tool calls from keeping parsed modules in memory between calls. With the store on, a call on an unchanged file list does no parse work. The store keeps at most 4 file lists and 256 MiB of source. This is a temporary opt-out: it goes away when the store is stable. | on | `FALLOW_MCP_WARM_SESSION=0` |
