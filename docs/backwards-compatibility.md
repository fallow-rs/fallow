...
- **`plugin-config-unreadable` on a source file that calls the Federation runtime** (2026-09-24): the Module Federation plugin now reads `registerRemotes` and `loadRemote` calls in a file that imports them from `@module-federation/runtime` or `@module-federation/enhanced/runtime`. A call whose argument is not a static literal records a `plugin-config-unreadable` entry whose `path` is that source file, `plugin` is `module-federation`, `key` is the function name (`registerRemotes` or `loadRemote`) and `reason` is the new token `dynamic-argument`. Both the `key` set and the `reason` set were already open, so no `schema_version` moves and no consumer change is required: the GitHub Action, the GitLab template and the MCP tools select on `degrades_analysis`, and a saved envelope that carries the new values loads in `fallow report --from`. A consumer that reads `path` as a config file sees a source file for these entries.

- **Two more Federation runtime functions, and SFC script blocks** (2026-09-25): the Module Federation plugin also reads `init` and `createInstance` calls, whose options declare `remotes`, and reads the runtime calls in the `<script>` blocks of a `.vue` or `.svelte` file. A `plugin-config-unreadable` entry and a trace `sources` entry can now carry `key` `init` or `createInstance`, and `path` or `config` can name a `.vue` or `.svelte` file. A `plugin-config-unreadable` entry and a trace source can also name a helper module that a bundler config imports, where they named only a config file before. The `key` set is open, so no `schema_version` moves and no consumer change is required.

- **Trace output names the Module Federation source** (2026-09-24): the `--trace-file` JSON (`FileTrace`) and the `--trace-dependency` JSON (`DependencyTrace`) gain an optional `sources` array, with the same field on the MCP `trace_file` and `trace_dependency` tools and the programmatic trace API. Each entry is a `TraceSource` with `kind` (`module-federation`; the set is open), `plugin` (the plugin that read the config, such as `module-federation` or `webpack`), `config` (the root-relative config file) and `key` (`exposes` for an exposed file, `remotes` for a remote alias; the set is open). A remote that only a literal runtime call names has an entry whose `config` is the source file and whose `key` is `registerRemotes` or `loadRemote`. A file trace lists each config whose `exposes` makes the file an entry point. A dependency trace lists each config that declares the name as a `remotes` alias, which marks the name as a remote container rather than an npm package. The array is omitted when it is empty, so a trace with no Federation involvement is byte-identical to before. No field changes meaning, no `schema_version` moves, and no consumer change is required.
...
- **Feature-flag envelope carries an optional `retirement` object** (2026-09-25): `fallow flags --retirement --format json` adds a top-level `retirement` object with one row per flag, the reasons the flag can be retired, and its age from git. Without `--retirement` the key is omitted, the envelope is byte-identical, and `schema_version` stays at `8` under the additive-optional-field rule above. The per-site `feature_flags[]` array is the same with and without the option. `retirement.flags[].reasons` and the `summary.by_reason` keys are an open set of kebab-case codes; a consumer must ignore a code it does not know. Every `retirement.flags[].actions[]` entry has `auto_fixable: false`. Two new open-set `workspace_diagnostics[].kind` values record why a flag age is missing: `flag-age-shallow-clone` and `flag-age-unavailable` (with `cause` `not-a-repository` or `no-commits`, an open set). Both answer `degrades_analysis: true` and appear only with `--retirement` and a `--flag-age` mode other than `off`. `--flag-state` adds `retirement.vendor_state` and a per-row `vendor` object, and rows of the new open-set kind `vendor_export`; a consumer must ignore a kind it does not know. The gates add `retirement.regression` and `retirement.max_flag_age` only when they run. Both carry a `status` of `pass`, `exceeded` or `skipped`; a `skipped` gate never sets exit code 1. `retirement.max_flag_age` is `skipped` when no flag age was measured (a shallow clone, no repository, or the API with `flag_age` off), and its `unmeasured` count gives the flags in scope without an age. `retirement.summary.distinct_flags` counts the flags in the code only, not the `vendor_export` rows. The regression baseline file gains an optional `flags` section; `REGRESSION_SCHEMA_VERSION` stays at `2` because older builds ignore it. The regression options keep their no-op behavior on `fallow flags` without `--retirement`, so no existing pipeline changes its exit code. SARIF adds the rule id `fallow/flag-retirement-candidate` (level `note`), CodeClimate adds the check name `fallow/flag-retirement` (severity `info`), and compact adds the `flag-retire:` prefix; each appears only with `--retirement`.
