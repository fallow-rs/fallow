...
- **`--fail-on-issues` raises `warn` complexity findings** (2026-09-24): with `--fail-on-issues` (or `--ci`, which sets it), `fallow health`, the bare `fallow` command and `fallow audit` write `effective_severity: "error"` on a complexity finding whose `complexity-*` rule is `warn`, as the flag already does for dead-code findings. The `health-findings` gate and the audit verdict then fail on it. `fallow audit` read no `--fail-on-issues` before; it now raises `warn` dead-code and complexity findings, so `fallow audit --ci` with only `warn` findings has verdict `fail` and exits 1 where it exited 0. The GitHub Action and the GitLab template do not pass the flag to the CLI, so they are not affected. The JSON shape does not change.
