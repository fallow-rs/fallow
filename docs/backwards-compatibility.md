...
- **Trace output names the Module Federation source** (2026-09-24): the `--trace-file` JSON (`FileTrace`) and the `--trace-dependency` JSON (`DependencyTrace`) gain an optional `sources` array, with the same field on the MCP `trace_file` and `trace_dependency` tools and the programmatic trace API. Each entry is a `TraceSource` with `kind` (`module-federation`; the set is open), `plugin` (the plugin that read the config, such as `module-federation` or `webpack`), `config` (the root-relative config file) and `key` (`exposes` for an exposed file, `remotes` for a remote alias; the set is open). A file trace lists each config whose `exposes` makes the file an entry point. A dependency trace lists each config that declares the name as a `remotes` alias, which marks the name as a remote container rather than an npm package. The array is omitted when it is empty, so a trace with no Federation involvement is byte-identical to before. No field changes meaning, no `schema_version` moves, and no consumer change is required.
