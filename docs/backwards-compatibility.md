...
- **Health config fields**: `health.coverage` and `health.coverageRoot` are stable fallbacks for standalone health, bare combined mode, and `fallow audit` (both the head pass and the base attribution pass) when the matching CLI flag and env var are omitted.
