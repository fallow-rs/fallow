...
- **`ignoreFindings` semantics**: patterns are validated as project-root-relative globs, leading `!` patterns keep reportable exceptions, and findings with multiple source owners are hidden only when every owner matches
