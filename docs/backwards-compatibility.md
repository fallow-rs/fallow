...
- **Audit and combined workspace diagnostics**: three envelopes gained `workspace_diagnostics[]` entries (issue #2366). `fallow audit --format json` populates `dead_code.workspace_diagnostics[]`; the `audit-brief` envelope shared by `fallow review --format json` and `fallow audit --brief --format json` populates the same path; and the bare combined `fallow --format json` gained a NEW top-level `workspace_diagnostics[]`. The two audit-family paths are the same `CheckOutput` array the standalone `dead-code` envelope already documents, so populating them is additive. The combined root field is a new optional field that is absent when the run records no diagnostics, which is additive under the rule above. All three use root-relative paths and are omitted when empty, and none bumps `schema_version`. Consumers that assumed the array was always absent in those three envelopes now see workspace-discovery, source-discovery, and analysis-stage entries there; the combined envelope's `check`, `dupes`, and `health` sections never carry the array.
