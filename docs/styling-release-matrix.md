...
| PandaCSS | Detects `defineTokens` token definitions, `token(...)` consumers, raw-style nearest-token suggestions, and token blast-radius. | Supports generated `styled-system` token imports, including path-aliased specifiers that still contain the `styled-system` segment. |
