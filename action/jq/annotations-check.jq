...
[
  ...
  (.unused_dependency_overrides[]? |
    "::warning file=\(.path | san),line=\(.line),title=Unused dependency override::Override `\(.raw_key | san)` forces `\(.target_package | san)` to `\(.version_range | san)` but no workspace package depends on `\(.target_package | san)`.\(nl)\(nl)\(if .hint then (.hint | san) + ".\(nl)" else "" end)Delete the entry, or scope it under a real parent (`pkg>\(.target_package | san)`) if it pins a transitive."),
  (.misconfigured_dependency_overrides[]? |
    "::error file=\(.path | san),line=\(.line),title=Misconfigured dependency override::Override `\(.raw_key | san)` -> `\(.raw_value | san)` is malformed (\(.reason | san)). `pnpm install` will reject this entry.\(nl)\(nl)Fix the key/value to match pnpm's override grammar, or remove the entry.")
] | .[]
