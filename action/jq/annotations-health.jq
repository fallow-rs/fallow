...
[
  (.findings[]? |
    ...
    (.effective_thresholds.max_cyclomatic? // $cyc_t) as $f_cyc |
    (.effective_thresholds.max_cognitive? // $cog_t) as $f_cog |
    (.effective_thresholds.max_crap? // $crap_t) as $f_crap |
    (if .crap != null then "  \u2022 CRAP: \(.crap | san) (threshold: \($f_crap | san))\(nl)" else "" end) as $crap_line |
    if .exceeded == "crap" or .exceeded == "cyclomatic_crap" or .exceeded == "cognitive_crap" or .exceeded == "all" then
      "::\($level) file=\(.path | prop),line=\(.line | n(1)),col=\((.col | n(0)) + 1),title=High CRAP score (\($sev | pval))::Function '\(.name | san)' has a CRAP score of \(.crap | san) (threshold: \($f_crap | san)).\(nl)\(nl)  \u2022 Severity: \($sev | san)\(nl)  \u2022 Cyclomatic: \(.cyclomatic | san)\(nl)  \u2022 Cognitive: \(.cognitive | san)\(nl)\($crap_line)  \u2022 Lines: \(.line_count | san)\(nl)\(nl)CRAP combines complexity with coverage: high CRAP means changes here carry high risk.\(nl)Consider adding tests, simplifying the function, or both."
    elif .exceeded == "both" then
      "::\($level) file=\(.path | prop),line=\(.line | n(1)),col=\((.col | n(0)) + 1),title=High complexity (\($sev | pval))::Function '\(.name | san)' exceeds both complexity thresholds:\(nl)\(nl)  \u2022 Severity: \($sev | san)\(nl)  \u2022 Cyclomatic: \(.cyclomatic | san) (threshold: \($f_cyc | san))\(nl)  \u2022 Cognitive: \(.cognitive | san) (threshold: \($f_cog | san))\(nl)\($crap_line)  \u2022 Lines: \(.line_count | san)\(nl)\(nl)Consider splitting this function into smaller, focused functions."
    elif .exceeded == "cyclomatic" then
      "::\($level) file=\(.path | prop),line=\(.line | n(1)),col=\((.col | n(0)) + 1),title=High cyclomatic complexity (\($sev | pval))::Function '\(.name | san)' has \(.cyclomatic | san) code paths (threshold: \($f_cyc | san)).\(nl)\(nl)  \u2022 Severity: \($sev | san)\(nl)  \u2022 Cyclomatic: \(.cyclomatic | san)\(nl)  \u2022 Cognitive: \(.cognitive | san)\(nl)\($crap_line)  \u2022 Lines: \(.line_count | san)\(nl)\(nl)High cyclomatic complexity means many branches to test.\(nl)Consider extracting conditionals or using early returns."
    else
      "::\($level) file=\(.path | prop),line=\(.line | n(1)),col=\((.col | n(0)) + 1),title=High cognitive complexity (\($sev | pval))::Function '\(.name | san)' is hard to understand (cognitive: \(.cognitive | san), threshold: \($f_cog | san)).\(nl)\(nl)  \u2022 Severity: \($sev | san)\(nl)  \u2022 Cyclomatic: \(.cyclomatic | san)\(nl)  \u2022 Cognitive: \(.cognitive | san)\(nl)\($crap_line)  \u2022 Lines: \(.line_count | san)\(nl)\(nl)High cognitive complexity means deeply nested or interleaved logic.\(nl)Consider flattening control flow or extracting helper functions."
    end),
  (.runtime_coverage.findings[]? |
    ...
    (if .evidence.test_only_reference == true then (.evidence.static_status + " (referenced only from tests)") else .evidence.static_status end) as $static |
    "::\($level) file=\(.path | prop),line=\(.line | n(1)),title=Runtime coverage (\(.verdict | pval))::Function '\(.function | san)' is flagged by runtime coverage.\(nl)\(nl)  \u2022 Verdict: \(.verdict | san)\(nl)  \u2022 Invocations: \($invocations | san)\(nl)  \u2022 Confidence: \(.confidence | san)\(nl)  \u2022 Static: \($static | san)\(nl)  \u2022 Tests: \(.evidence.test_coverage | san)\(nl)  \u2022 V8: \($tracking | san)\(nl)\(nl)\(if .actions | length > 0 then .actions[0].description | san else "Review the runtime evidence before changing this path." end)"),
