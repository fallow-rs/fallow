...
if $total == 0 then
  ...
  (if $check > 0 then
    ...
    ([
      ...
      (if ((.check.dev_dependencies_in_production // []) | length) > 0 then "| [Dev dependencies in production](\(docs("dev-dependencies-in-production"))) | \(.check.dev_dependencies_in_production | length) |" else null end),
