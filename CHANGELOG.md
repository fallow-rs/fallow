...
- **Inner complexity-finding struct renamed from `HealthFinding` to `ComplexityViolation` ahead of the #384 B2 wrapper migration.** Prerequisite for the upcoming `HealthFinding` wrapper that will flatten the inner struct via `#[serde(flatten)]` and add typed `actions` + `introduced` natively, retiring `inject_health_actions` from the JSON post-pass. Renaming up front avoids landing the wrapper under a `HealthFindingFinding` collision. The struct, every Rust consumer in `crates/cli` and `crates/types`, the schemars derive, `docs/output-schema.json`'s `$ref` graph, and the generated TS contracts (`editors/vscode/src/generated/output-contract.d.ts`, `npm/fallow/types/output-contract.d.ts`) all flip to `ComplexityViolation`. The JSON wire-level field name on `HealthReport.findings[]` (and its grouped / audit / combined echoes) is controlled by serde, so wire shape stays byte-identical; verified against the `vue-core` benchmark fixture (`total_issues`, `findings[]` ordering, and per-finding `actions[]` discriminants identical, only `elapsed_ms` differs). `editors/vscode/src/types.ts` carries an `export type HealthFinding = ComplexityViolation` backwards-compat alias following the existing `*Finding` wrapper pattern so downstream consumers that import the legacy name keep compiling. `HealthFindingAction` and `HealthFindingActionType` keep their names; they describe actions attached to the finding and naturally fit the future wrapper. (Refs [#384](https://github.com/fallow-rs/fallow/issues/384))
